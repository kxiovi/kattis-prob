...
The secret tests cases:
    ...
    01: number of max gifts possible is 5400. so if  
    gift count is above that, it's impossible  

    02: overflows if the solution doesn't use a 64-bit integer  
    for storing gift count g  

    03: stress case, max of all inputs. This wouldn't be  
    too stressful if code accounts for 5400 gifts possible  

    04: tests the minimum of all inputs  
    
    05: tests the max of all inputs except g is 5400  
    in the case that solution has a trick to exit at  
    a certain threshold of g  

    06: random, just so it may somehow catch  
    something missed by other test cases  

    07: a test that should output `possible` because  
